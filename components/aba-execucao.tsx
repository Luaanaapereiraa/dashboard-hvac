...
interface AbaExecucaoProps {
  equipamentos: EquipamentoExecucao[]
  onUpdateEquipamento: (id: string, campo: keyof Omit<EquipamentoExecucao, "id" | "tag" | "setor" | "temEvidencia">, valor: number) => void
  onUploadEvidencia: (equipId: string) => void
}

const colunas: { key: keyof Omit<EquipamentoExecucao, "id" | "tag" | "setor" | "temEvidencia">; label: string; abrev: string }[] = [
  { key: "infra", label: "Infraestrutura", abrev: "INF" },
  { key: "tubulacao", label: "Tubulação", abrev: "TUB" },
  { key: "instalacaoMaquina", label: "Inst. Máquina", abrev: "MAQ" },
  { key: "eletrica", label: "Elétrica", abrev: "ELE" },
  { key: "comissionamento", label: "Comissão", abrev: "COM" },
]
...
export function AbaExecucao({ equipamentos, onUpdateEquipamento, onUploadEvidencia }: AbaExecucaoProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground">Tabela de Avanço Físico</h2>
          <p className="text-sm text-muted-foreground">
            Use Tab para navegar • Setas para mover • Enter para confirmar
          </p>
        </div>
          ...
          <Table className="text-xs">
            <TableHeader>
              <TableRow className="bg-muted/50 hover:bg-muted/50">
                <TableHead className="w-[100px] font-semibold text-xs">TAG</TableHead>
                <TableHead className="w-[140px] font-semibold text-xs">Setor</TableHead>
                {colunas.map((col) => (
                  <TableHead key={col.key} className="w-[70px] text-center font-semibold text-xs" title={col.label}>
                    {col.abrev}
                  </TableHead>
                ))}
                <TableHead className="w-[50px] text-center font-semibold text-xs">Média</TableHead>
                <TableHead className="w-[80px] text-center font-semibold text-xs">Evidência</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {equipamentos.map((equip) => {
                ...
                return (
                  <TableRow key={equip.id} className="group hover:bg-muted/20">
                    <TableCell className="font-medium text-foreground py-1">
                      <code className="text-[10px] bg-muted px-1 py-0.5 rounded">
                        {equip.tag}
                      </code>
                    </TableCell>
                    <TableCell className="text-muted-foreground text-xs py-1 truncate max-w-[140px]" title={equip.setor}>
                      {equip.setor}
                    </TableCell>
                    ...
                    <TableCell className="text-center py-1">
                      <Button
                        variant={equip.temEvidencia ? "ghost" : "outline"}
                        size="sm"
                        onClick={() => onUploadEvidencia(equip.id)}
                        className={cn(
                          "h-6 w-6 p-0",
                          equip.temEvidencia && "text-status-success hover:text-status-success"
                        )}
                        title={equip.temEvidencia ? "Evidência anexada" : "Anexar evidência"}
                      >
                        {equip.temEvidencia ? (
